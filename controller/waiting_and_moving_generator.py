...
class WaitingAndMovingEdgesGenerator (TimeWindowGenerator4Benchmark):
    def __init__(self, dm):
        super().__init__(dm) 
        self._graph = None
        self.rerouting_controller = ReroutingController(self)
    ...
    def write_to_file(self, agv_id_and_new_start=None, new_halting_edges=None,
        ...
        if agv_id_and_new_start is not None:
            if agv_id_and_new_start[1] == 855:
                pdb.set_trace()
        self.rerouting_controller.write_to_file(
            agv_id_and_new_start, new_halting_edges,
            supply, vs_id, vt_id, filename
        )